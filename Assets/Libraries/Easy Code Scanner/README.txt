...
A : - Make sure you have declared the 2 activities of the plugin in your Manifest and also the set the permissions (CAMERA & VIBRATE).
    - Make sure all the libs are in Assets/Plugins/Android
    - If you was using an old version of the plugin, after upgrading the plugin, close your project, remove the Library and Temp directory at the root of your project. Re-open and rebuild.
    
Q : On Android, when building, I have errors like "Unable to convert classes into dex format"
A : Some library are probably present more than once. Old version of this plugin contained a directory named "project" in Assets/Plugins/Android.
    This directory is not needed because it's only the source of our plugin to customize the native interface.
    If present, remove it then rebuild.
    If the problem is still present, close your project, remove the Library and Temp directory at the root of your project. Re-open and rebuild.
